...
function showAuthPage(page) {
  document
    ...
    .classList.toggle("hidden", page !== "login");
  document
    .getElementById("page-register")
    .classList.toggle("hidden", page !== "register");
  // Clear inputs
  [
    "login-identifier",
    "login-password",
...
function refreshDashboard() {
  ...
  const lang = getLang();
  const greeting =
    lang === "id" ? `Halo, ${user.username}!` : `Hello, ${user.username}!`;
...
function getSchedMode() {
  ...
  if (!user) return 5;
  return parseInt(
    localStorage.getItem(`lumos_schedmode_${user.username}`) || "5",
  );
}
...
function updateThemeIcons(theme) {
  ...
  document
    .getElementById("mobile-icon-moon")
    ?.classList.toggle("hidden", isDark);
  document
    .getElementById("mobile-icon-sun")
    ?.classList.toggle("hidden", !isDark);
}
...
function handleReviewQuiz() {
  ...
  const isID = getLang() === "id";
  reviewEl.innerHTML = currentQuestions
    .map((q, idx) => {
      const ua = userAnswers[idx];
      const ca = q.answer.toUpperCase();
